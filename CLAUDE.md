...
- [`019-parakeet-asr-deferred.md`](docs/decisions/019-parakeet-asr-deferred.md) — **Deferred** (researched, not built): keep Whisper as the sole transcription engine. Parakeet-TDT (higher multilingual ASR quality) **is** portable — `mlx-community/parakeet-tdt-0.6b-v3` weights + `senstella/parakeet-mlx` (Python MLX ref) + the in-house Sortformer FastConformer encoder — BUT the MIT Swift-MLX reference (`FluidInference/swift-parakeet-mlx`) was **archived for performance** (sequential TDT/RNN-T decode too slow in MLX-Swift; they moved to CoreML), and CoreML is **not Metal-governed** (breaks the ADR-011 thesis, like the WhisperKit rejection). So the fast option breaks the thesis and the thesis-aligned option carries an unquantified perf risk. Interim: a Parakeet/non-Whisper transcription id is refused with a cause-naming 400 `unsupported_transcription_arch` (`TranscriptionArch` denylist, v0.10.170), not an opaque 500. Tripwire: reopen for a **spike-first** benchmark if multilingual-ASR demand justifies a milestone, or if a fast MLX TDT decode appears.

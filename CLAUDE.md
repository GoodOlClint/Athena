...
- [`020-multi-backend-transcription.md`](docs/decisions/020-multi-backend-transcription.md) — **Proposed** (M76, pending review): make transcription **multi-backend, model-class routed** (Whisper + Parakeet), mirroring diarization's ADR 018. Extend `TranscriptionArch` from a denylist into a positive router (`whisper`/`parakeet`/`unsupported` from `config.json`); `MLXTranscriptionModule` dispatches `loadModel` to the Whisper or Parakeet engine; single governed `transcription` slot (ADR 011). **Whisper stays default; Parakeet additive** (selected by `model=`, operator-pulled) — whether Parakeet *becomes* default is an **open decision** gated on a WER/throughput A/B. `/v1/audio/transcriptions` surface unchanged; Parakeet word/segment timestamps from TDT durations (0.08 s/frame). Harden the ADR-019 spike (mel-exactness vs the reference int16-view magnitude, governor integration, SentencePiece detokenizer, long-audio chunking, tests). No CoreML (thesis). Plan: `docs/parakeet-transcription-plan.md`.

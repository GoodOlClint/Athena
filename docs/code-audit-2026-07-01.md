...
>
> **Live e2e verification (2026-07-02, real models, HEAD binary):** `e2e-rbac.sh` **496/0**, `e2e-tool-choice-auto.sh` **7/7** and `e2e-anthropic-messages.sh` **5/5** (both on `gemma-4-12B-it-8bit`), `e2e-video.sh` **PASS** (exercises the reworked gated `transcribePCM`). Unrelated pre-existing issue surfaced while running them: the **gemma-4 e2b/e4b checkpoints fail to load** (`keyNotFound: …self_attn.k_norm.weight` via the Gemma4 VLM path) — a substrate arch-coverage gap for the e-variant (MatFormer) layout, tracked in memory; dense 12B and the studio's 26B MoE load fine.
>

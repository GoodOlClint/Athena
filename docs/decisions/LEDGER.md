...
- [ADR 035 — Route channel-delimited reasoning to `reasoning_content`](035-reasoning-channel-to-reasoning-content.md) — Accepted — implemented; amended #198 (close-tag-only reasoning form, prompt-derived — not model-gated; `<think>` recognized only when it opens the completion)

...
> **Amendment, 2026-07-25 — the chown alone did not work; the write had to change.** Found in the field on the Mac Studio appliance: every daemon-mediated config write failed `writeFailed` with `NSPOSIXErrorDomain 13` from `mktemp`. Chowning the FILE is necessary but insufficient, because an atomic write does not write the file — it creates a temp file **in the containing directory** and renames over it, and `/usr/local/etc/athena` is `root:wheel`. So the exact failure this ADR set out to eliminate survived its own fix, invisibly, until someone tried `athena config set` against an installed daemon.
>
> **The directory stays root-owned — that part is deliberate, not an oversight to fix.** `auth_keys_file` and the TLS private key live in that same directory by default (the `DefaultConfig` template points at `/usr/local/etc/athena/auth.keys` and `.../tls/privkey.pem`). A service-writable directory would let a compromised daemon *replace* those files — unlink+create needs only directory write — which would silently defeat §4's deny-list, whose whole point is that the daemon cannot repoint `auth_keys_file`/`tls_key`. Widening the directory to fix a write bug would have traded the ADR's security decision for its usability one.
>
> **Fix: `ConfigEditor` writes in place when the atomic write fails**, which needs only the file's own write bit — the bit the install already grants. Symmetrically, it now **restores the file's prior uid/gid after an atomic write**, because a single `sudo athena config set` otherwise replaces the inode with a root-owned one and permanently re-breaks the daemon's writes (also observed on the Studio: the sudo fallback used to work around the bug re-rooted the file). Trade-off accepted: an in-place write is not crash-atomic, bounded by the existing NB2 rollback (a write that makes a previously-valid config unparseable is reverted) and by the file being a few hundred bytes. Pinned by `ConfigEditorTests` — a read-only directory containing a writable file, which fails against the pre-fix code.

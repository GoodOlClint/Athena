...
**Implemented:** M66.5 (v0.10.127). `--password` removed from `auth user add`,
`auth user passwd`, `proxy login` (macOS) and the portable `auth user add`
(client). Secrets now come from `--password-stdin`, `$ATHENA_PASSWORD`, or an
interactive prompt via a shared `PasswordInput.resolve` (macOS) /
`RemoteAuth.resolvePassword(stdin:)` (client). The e2e suite was migrated to
`ATHENA_PASSWORD`. **K7** (`Credentials` Keychain `security -w <value>`) and
**K13** (`--key` in argv) are the remaining argv-secret items — audited here,
tracked in M71 (they need a Keychain-API / `--key`-steering change, out of
scope for the `--password` removal).

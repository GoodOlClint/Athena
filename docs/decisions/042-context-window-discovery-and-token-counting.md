...
**Status:** Accepted — **PLANNED, not implemented.** Brownfield-gated; operator-approved 2026-07-25 (surface shape confirmed by interview).
**Date:** 2026-07-25
...
The Anthropic `POST /v1/messages/count_tokens` from §3 is **retained in this ADR but deferred** — same counting core, a decoder that already exists, ~one thin handler. It lands when a Messages-dialect consumer needs it (Claude Code degrades gracefully without it: no context meter, no error). Building it now would be speculative.

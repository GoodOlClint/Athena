...
1. `MemoryGovernor.rebind(_:to:perform:)` admits a warm swap in place of the old reservation (the new model's estimate minus the old model's bytes), then reconciles to the module's post-swap self-report. The process-wide probe delta cannot attribute a swap, because the old model's buffers go to the MLX cache inside the same window. Every serve-path rebind (`auditedRebind`, `POST /api/models/load`) goes through it. Admission stays outside the ADR 029 gate, because the cache-reclaim rung takes the gate itself. The swap runs inside the gate.
...
Not covered: prefill activations beyond the KV cap (bounded by the ADR 030 prompt ceiling, not reserved here), and the MLX cache filling toward `memoryLimit` (reclaimable, excluded from `committed` by G2).

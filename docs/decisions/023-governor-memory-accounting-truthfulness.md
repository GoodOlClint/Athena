...
## Amendment (2026-09-24) — rebinds, per-model estimates, and KV headroom (#206)

G2 and G3 as shipped assumed every load went through `performLoad`. They did not cover three paths, and a field daemon crossed its budget through all three (#206: `phys_footprint` 103.3 GB and 110.8 GB against a 103.08 GB budget).

- **Rebinds bypassed the governor.** A warm swap (`loaded → loaded`, another model) never reached admission or reconcile, so a slot admitted for a small model could load a large one unchecked, and `residentBytes` kept the first load's number. Measured on a 30 GB budget: with an 8 GB embedder resident, a rebind to a 24.6 GB model took `phys_footprint` to 33.3 GB and evicted nothing, while `residentBytes` still read 24.9 GB.
- **Estimates were per module class, not per model.** The LLM estimate was the largest LLM in the store, fixed at startup. The learned footprint was keyed by module alone. The store maximum refused a 3.4 GB model on a budget that held a larger model in the store (503, requested 36.6 GB). The first load's learned number admitted every later model.
- **Weights that fit, with no room for their KV, cross on the first long prompt.** A single 24.6 GB model admitted against 30 GB reached `phys_footprint` 30.95 GB (MLX active 30.05 GB, cache 0.63 GB) on a 23k-token prompt. The overshoot is KV and prefill activations, not the reclaimable cache.

Decision:

1. `MemoryGovernor.rebind(_:to:perform:)` admits a warm swap in place of the old reservation (the new model's estimate minus the old model's bytes). While the swap runs it holds `max(old, new)`, because either model may be resident until it finishes. It then reconciles to the module's post-swap self-report, only if the target is actually resident. The process-wide probe delta cannot attribute a swap, because the old model's buffers go to the MLX cache inside the same window. A swap that fails with the old model still resident restores the old reservation. One that leaves the slot empty releases it. Rebinds of one slot run one at a time. The server's rebinds (`auditedRebind`, `POST /api/models/load`) go through it. Admission stays outside the ADR 029 gate, because the cache-reclaim rung takes the gate itself. The swap runs inside the gate. A load or swap that evicted tenants waits for their teardown before allocating, with its own reservation already recorded.
2. Admission estimates the requested model: `ModelSelectable.admissionEstimate(forModel:)` returns the target's on-disk safetensors bytes (LLM and text-embedding). The learned footprint is keyed by `(module, model)`. The LLM's `residentBytes` is the loaded parameter bytes, not a startup constant.
3. While the LLM is resident, admission keeps one request's KV headroom free: `promptCacheCapBytes`, the per-request KV bound the LLM already enforces at preflight. This applies to the LLM's own load and rebind and to any co-tenant load. It rides the G2 revert switch: `governor_admission_mode = "estimate"` turns it off.
4. A model that can never fit is refused before any load with `400 model_exceeds_memory_budget`: its weights exceed the budget, or for the LLM, its weights plus the KV headroom do. A load that cannot fit now because of resident tenants is still `503 memory_budget_exceeded`.

Not covered: prefill activations beyond the KV cap (bounded by the ADR 030 prompt ceiling, not reserved here); the MLX cache filling toward `memoryLimit` (reclaimable, excluded from `committed` by G2); a paired MTP drafter on the target's first load (its bytes join the self-report and so the learned footprint, but not the first on-disk estimate); and module-internal rebinds that run inside the gate (the LLM decode's own `rebind` when concurrent requests name different models, and `embed` when a request omits `model`). Those cannot call admission from inside the gate, since the reclaim rung takes it; tracked in #217. The KV headroom refuses LLMs whose weights exceed `budget − prompt_cache_cap_bytes`, which on the default sizing (budget 0.75 × RAM, cap budget/4) is about 0.56 × RAM (about 27 GB on a 48 GB machine). An operator who wants such a model lowers `prompt_cache_cap_bytes` (this also caps the prompt length) or sets `governor_admission_mode = "estimate"`.

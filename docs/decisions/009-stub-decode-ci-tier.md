...
| **M48.3** temperature inertness under a Guide | **empty — there is no CI half, and that is not an omission.** The contract is that `GuidedSubstrate` decodes with a hardcoded `ArgMaxSampler()`, a property only a real MLX decode exhibits; there is no MLX-free decision to extract, since `DecodeDispatch.route` already pins the *routing* (L5 covers the mask seam). Recorded as empty rather than left out, per this section's no-silent-caps rule. | `GuidedTemperatureInertnessTests.testTemperatureIsInertUnderGuide` (`ATHENA_RUN_MODEL_TESTS=1`) — **restored by #65**: a schema request at temperature 0.1 must emit the byte-identical sequence a temperature-0 request does. Distinct from both struck rows above: L1 was bit-identity across `speculative`, L2 was the acceptance-rate floor. |

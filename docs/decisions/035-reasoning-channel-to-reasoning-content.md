...
**Status:** Accepted (gate-approved 2026-06-30) — **IMPLEMENTED v0.10.232** (`deabfe69`); **amended 2026-09-23/24 (#198)** for the close-tag-only reasoning form (Qwen3.5's `<think>…</think>`) — see "Amendment" below, and the operator-ruled "Correction (round 3)" that keeps "Universal, not model-gated" true by deriving the decision from the rendered prompt instead of a model-name guess. This line read "implementing" from the moment the code landed; corrected #193 against the tree. Live: `ReasoningChannelFilter` + `splitReasoningChannel` (Gemma), `QwenThinkFilter` + `splitQwenThink` + `qwenReasoningMode` (close-tag-only form) — all in `Sources/AthenaServerKit/ReasoningChannel.swift`; `ReasoningPromptTail` (the prompt-derived signal) in `Sources/AthenaCore/ReasoningPromptTail.swift`.
**Date:** 2026-06-30

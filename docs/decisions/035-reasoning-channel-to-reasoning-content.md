...
**`QwenThinkFilter.Mode` simplifies from three cases to two.** `.disabled` (the round-2 fix for "don't scan for `<think>` outside Qwen3.5") is gone — there is no "outside Qwen3.5" concept to gate on anymore. `.awaitingOpenTag` (renamed from nothing — it already existed, formerly reached only via the model-name gate returning non-`.disabled`) is now simply the default for every request whose prompt didn't start already open; it is the SAME marker-driven no-op design `ReasoningChannelFilter` already relies on for Gemma, so the residual risk (a model's ordinary content happening to contain the literal string `<think>`) is not new — it's the risk ADR 035 already accepted for `<|channel>`, extended to one more marker pair. (A third mode returns in round 4 below, for an unrelated reason — structured-payload integrity, not model gating.)

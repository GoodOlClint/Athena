...
- **Non-vision, plain-text chat only.** Batch iff `batching_enabled && schemaJSON==nil && (tools==nil||empty) && logprobs==nil && speculative != true && !residentIsVision`. (VLM-container `.model` under BatchGenerator is unverified — the daemon loads gemma4 via `VLMModelFactory`, keeping the tower; the spike batched it via the *text* factory. Verify separately before enabling batching for vision models.) Everything else → the existing per-request path, unchanged.

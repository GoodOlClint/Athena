...
- In-memory data protection (process hardening: Hardened Runtime, no `get-task-allow`, `RLIMIT_CORE=0`, opt-in debugger denial, PCM zeroization) and at-rest store encryption are documented in `docs/at-rest.md` and the ADRs under `docs/decisions/` (024). The honesty boundary is stated there: the active working set is irreducibly plaintext in RAM, and a kernel/root adversary is out of scope.

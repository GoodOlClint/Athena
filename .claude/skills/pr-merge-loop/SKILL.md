...
- Push the branch per **AGENTS.md "Agent pushes go through the GitHub MCP"** (operator decision 2026-08-31): `create_branch` + `push_files` as the App (commit message goes to the tool; no `Co-Authored-By` trailer — the App is the author), then the byte-verify — commit the identical change locally, `git fetch`, and `git diff <local-commit> origin/<branch> --` must be empty. `push_files` cannot express deletions or renames; a deletion goes through `delete_file` (one path per call) and a rename is `push_files` of the new path plus `delete_file` of the old, per the same AGENTS.md section (same byte-verify gate; a mode-bearing rename that fails the verify falls back to operator-attended local `git push`). `.github/workflows/*` changes push through the same API path — the App token carries `workflows` permission (AGENTS.md "Agent pushes go through the GitHub MCP"); the real gate on a workflow-touching change is operator PR approval, not the push. Then `create_pull_request` (base main). Do NOT arm auto-merge: the loop's job doesn't end at merge — the post-merge harvest and doc reconciliation need the agent present at merge time, and auto-merge can land the squash while the loop is between polls.

          ...
          "paths": {
            "/healthz": {
              "get": {
                ...
                "description": "Always open (no auth). Returns the governor's current reservations and budget, plus `inflight` (live request count) and `lastRequestAt` (unix-epoch seconds; 0 ⇒ none since boot) for at-a-glance diagnosis of a hung daemon (M43.1). M60.1 adds `thermalState` (`nominal`/`fair`/`serious`/`critical` from macOS thermal pressure — the throttle indicator to back off on), `lastDecodeTokensPerSec` (most recent decode throughput), and `mlxActiveBytes`/`mlxCacheBytes` (MLX allocator counters). M60.2 adds `powerAssertionHeld` (whether the daemon holds a PreventUserIdleSystemSleep assertion; `false` means an unattended Mac can sleep and suspend inference). M60.3 adds `gpuClockMHz`, `gpuActiveResidency`, and `gpuTemperatureC` (sudoless in-process GPU telemetry — clock/residency via IOReport, GPU die temperature in °C via SMC; each `null` when unavailable on the host). ADR 023 adds `mlxCacheLimitBytes` (G1 — the serve-path MLX buffer-cache bound; `mlxCacheBytes` should plateau at/under it) and `admissionMode` (G2 — `footprint` means admission/`freeBytes` are metered against the real Metal footprint `budget − max(committed, reserved)`; `estimate` is the reservation-only revert switch). ADR 038 adds inference-execution-gate observability: `gateWaiters` (requests queued behind the ADR-029 serialization gate), `gateHeld`/`gateHeldMs` (whether it is held and for how long — a long transcription/convert ahead of chat is visible here), `gateMaxWaiters` (high-water depth since boot), and `gateWaitP95Ms` (recent queue wait). Routine `gateWaiters ≥ 2` / rising `gateWaitP95Ms` is the evidence trigger for continuous batching.",
                "security": [],
                "responses": {
            ...
            "/metrics": {
              "get": {
                ...
                "description": "Global, process-lifetime counters (reset on restart). Content-negotiated: returns Prometheus text-exposition format 0.0.4 by default (the scrape target), or the JSON snapshot when the request sends `Accept: application/json`. Requires the `metrics.read` permission. ADR 038 adds `athena_inference_gate_*` series to the Prometheus output (JSON snapshot unchanged): `_waiters`/`_held`/`_held_ms`/`_max_waiters` gauges, `_acquisitions_total`/`_contended_total` counters, and the `_wait_ms` summary — queue depth + wait time behind the ADR-029 execution gate.",
                "responses": {
            ...
            "/v1/chat/completions": {
              "post": {
                ...
                "description": "OpenAI-compatible. Set `stream:true` for an SSE stream of `chat.completion.chunk` events terminated by `data: [DONE]`; with `stream_options.include_usage:true` a final usage-only chunk precedes it. `response_format` (`json_object`/`json_schema`) and a **forced** `tool_choice` (`required`, or a named tool matching a declared one — either forced form only with a non-empty `tools` array; an unknown name degrades to advertise-only, and an empty menu disables tools entirely) route into structured output; `tool_choice:\"auto\"` advertises the tool menu without engaging the Guide (ADR 034), so `tools` alone is not a structured request. `top_p`/`seed` are honored only on the sampling path and are inert under greedy/MTP/structured decoding. `logprobs`/`top_logprobs` (0–20) are honored when `temperature` compares equal to `0`, or when the request is schema-backed in the sense just described, and return `choices[].logprobs.content`. Otherwise `logprobs` ⇒ 400 `logprobs_requires_deterministic` — including an **omitted** `temperature`, which is an absent override rather than a request for greedy decoding, and including an underflowing value such as `-1e-60` that decodes greedily but is not zero. `n>1` and non-empty `logit_bias` are rejected 400. Requires `inference`.",
                "requestBody": {
                  "required": true,
            ...
            "schemas": {
              ...
              "ChatCompletionRequest": {
                ...
                "properties": {
                  ...
                  "logprobs": { "description": "Honored when temperature compares equal to 0, or when the request is schema-backed (response_format of json_object/json_schema, or a forced tool_choice of required/named, which engages only with a non-empty tools array (and, for the named form, a declared matching name) — tool_choice:auto does not count, ADR 034; an unknown name degrades to advertise-only and an empty menu disables tools entirely); returns choices[].logprobs.content. Otherwise ⇒ 400 logprobs_requires_deterministic, including an omitted temperature (an absent override, not a greedy request) and an underflowing value such as -1e-60 that decodes greedily but is not zero. Set temperature:0 explicitly, or make the request schema-backed." },

          ...
          "paths": {
            ...
            "/v1/chat/completions": {
              "post": {
                ...
                "description": "OpenAI-compatible. Set `stream:true` for an SSE stream of `chat.completion.chunk` events terminated by `data: [DONE]`; with `stream_options.include_usage:true` a final usage-only chunk precedes it. `response_format`/`tools` route into structured output. `top_p`/`seed` are honored only on the sampling path and are inert under greedy/MTP/structured decoding. `logprobs`/`top_logprobs` (0–20) are honored when `temperature` is explicitly `0`, or when the request is structured (`response_format`/`tools`), and return `choices[].logprobs.content`. Any other case with `logprobs` and no schema ⇒ 400 `logprobs_requires_deterministic` — including an **omitted** `temperature`, which is an absent override rather than a request for greedy decoding, and including a negative or underflowing value that decodes greedily but is not literally zero. `n>1` and non-empty `logit_bias` are rejected 400. Requires `inference`.",
                "requestBody": {
                  "required": true,
            ...
            "schemas": {
              ...
              "ChatCompletionRequest": {
                ...
                "properties": {
                  ...
                  "logprobs": { "description": "Honored when temperature is explicitly 0, or when the request is structured; returns choices[].logprobs.content. Anything else with logprobs and no schema ⇒ 400 logprobs_requires_deterministic — including an omitted temperature (an absent override, not a greedy request), and including a negative or underflowing value that decodes greedily but is not literally zero. Set temperature:0 explicitly, or use structured output." },
